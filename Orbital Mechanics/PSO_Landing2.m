...
global N_iterations GBest lambda a b tol land LU TU 
...
yph = X*sin(w) + Y*cos(w);
xangle=abs(max(xph)+min(xph))/2;
%% Initial DRO
r0=[AX+(1-lambda);0];
v=[0;VY];
yy=[r0;v];
orbitspan=linspace(0,T1,800);
options=odeset('RelTol',1e-10,'AbsTol',1e-10);
[~,orbit]=ode113(@orbitfunCR3BP,orbitspan,yy,options);
orbitpos=[orbit(:,1),orbit(:,2)];
orbitvel=[orbit(:,3),orbit(:,4)];
sizeorbit=size(orbitpos,1);
%% Landing site
land=[xph(86);yph(86)];
%-------------------------------------------------------------------------%
...
gamma = zeros(N_iterations,1); 
penalty = 100; % need to tweak this to make sure landing contraints are met 
% (i.e. s/c lands on Phobos)

tol = .1/LU; % 1 km tolerance
...
% where improvements can be made
 for p=1:sizeorbit
    angle(p)=mod(atan2(orbitpos(p,2),orbitpos(p,1)-xangle),2*pi);
 end
...
for j = 1:N_iterations
    ...
    for p=1:sizeorbit
        anglecheck(p,i)=abs(angle(p)-P(i,3));
    end
    [minangle(i),index]=min(anglecheck(:,i));
    vinitial=[orbitvel(index,1);orbitvel(index,2)];
    r0=[orbitpos(index,1);orbitpos(index,2)];
    if vinitial(2)<0 && vinitial(1)>=0
        if abs(vinitial(2))>=abs(vinitial(1))
            v0=[vinitial(1)-P(i,2)*cos(P(i,1));vinitial(2)+P(i,2)*sin(P(i,1))];
        else
            v0=[vinitial(1)-P(i,2)*cos(P(i,1));vinitial(2)-P(i,2)*sin(P(i,1))];
        end
    elseif vinitial(2)<0 && vinitial(1)<0
        if abs(vinitial(2))>=abs(vinitial(1))
            v0=[vinitial(1)-P(i,2)*cos(P(i,1));vinitial(2)+P(i,2)*sin(P(i,1))];
        else
            v0=[vinitial(1)+P(i,2)*cos(P(i,1));vinitial(2)+P(i,2)*sin(P(i,1))];
    ...
    [tof,yf]=ode113(@orbitfunCR3BP,timespan,y0,options);
    pos=[yf(:,1),yf(:,2)];
...
%     J(i)= penalty*abs(norm(posf)-norm(land)) + dvtot(i); 
    ...
    %evaluating cost function  
        if abs(norm(posf)-norm(land)) > tol 
            J(i)= gamma(j)*abs(norm(posf)-norm(land)) + dvtot(i);  
        else
...
filename = sprintf('PSO_Landing_2_%s.mat',time);
...
fprintf('Position angle along the orbit: %1.2f°\n',GBest(1,3)*180/pi)
fprintf('Time of Flight: %1.5f s',tof(end)*TU)
